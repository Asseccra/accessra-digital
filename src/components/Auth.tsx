...
export default function Auth({ darkMode, onLoginSuccess }: AuthProps) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (auth.currentUser) {
      ...
      await saveUserToFirestore(currentUser, email);

      const emailUser: UserProfile = {
        uid: currentUser.uid,
        email: currentUser.email || email,
        displayName: currentUser.displayName || emailDisplayName,
        photoURL:
        ...
        {authMode === 'otp' && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
            ...
            <div className="text-center">
              <button type="button" onClick={() => setAuthMode('login')} className="text-xs text-slate-500 hover:text-blue-600 underline cursor-pointer font-bold">
                Kembali ke Login
              </button>
            </div>
          </motion.div>
        ...
        {authMode === 'forgot' && (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            ...
            <form onSubmit={handleForgotPasswordSubmit} className="space-y-4">
              ...
              <button type="submit" className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-700 via-blue-600 to-cyan-500 text-white text-sm font-semibold cursor-pointer shadow-lg transition-all duration-300">
